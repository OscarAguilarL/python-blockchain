...
def get_balance(participant):
    ...
    amount_received = functools.reduce(
        lambda tx_sum, tx_amt: tx_amt[0] if len(tx_amt) > 0 else 0, tx_recipient, 0)
...
def add_transaction(_recipient, sender=owner, _amount=1.0):
    """
    ...
    Arguments:
        ...
        :amount: the amount of coins sent with the transaction (The default is 1)
    """
    transaction = {
        'sender': sender,
...
def get_transaction_value():
    ...
    tx_amount = float(input("Enter a transaction amount: "))
...
while waiting_for_input:
    ...
    if user_choice == '1':
        ...
        if add_transaction(recipient, _amount=amount):
